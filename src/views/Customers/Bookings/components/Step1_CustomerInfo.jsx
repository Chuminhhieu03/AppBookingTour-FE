...
import { Form, Input, Row, Col, Button, Space, Table, InputNumber, Select, DatePicker, message, Spin } from 'antd';
...
const Step1_CustomerInfo = ({ itemId, scheduleId, bookingType = 'combo', onComplete, loading }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoadingData(true);
                // TODO: Implement API calls for tour and accommodation
                const response = await comboAPI.getById(itemId);
                if (response.success) {
                    const combo = response.data;
                    setComboData(combo);

                    // Tìm schedule tương ứng
                    const schedule = combo.schedules?.find((s) => s.id === parseInt(scheduleId));
                    if (schedule) {
                        setScheduleData(schedule);

                        // Set giá mặc định
                        form.setFieldsValue({
                            priceAdult: schedule.basePriceAdult || combo.basePriceAdult,
                            priceChild: schedule.basePriceChildren || combo.basePriceChildren,
                            numAdults: 1,
                            numChildren: 0,
                            numSingleRooms: 0,
                            singleRoomSurcharge: schedule.singleRoomSupplement || 0
                        });
                ...
                message.error(`Đã xảy ra lỗi khi tải thông tin ${bookingType}`);
            } finally {
    ...
    // Tính tổng tiền - Lấy giá từ scheduleData thay vì form
    const calculateTotalAmount = () => {
        const priceAdult = scheduleData?.basePriceAdult || 0;
        const priceChild = scheduleData?.basePriceChildren || 0;
        const singleRoomSurcharge = scheduleData?.singleRoomSupplement || 0;
    ...
    // Table columns cho participants
    const columns = [
        {
            title: 'Họ tên',
            dataIndex: 'fullName',
    ...
    const handleSubmit = async () => {
        try {
            ...
            const bookingData = {
                ...
                travelDate: scheduleData.departureDate,
                ...
                singleRoomSurcharge: values.singleRoomSurcharge,
                    ...
                    <Row gutter={[16, 16]} align="middle">
                        ...
                        <Col span={12}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 12 }}>
                                ...
                                <Button icon={<PlusOutlined />} onClick={() => setNumAdults(Math.min(20, numAdults + 1))} />
                            </div>
                        </Col>
                        <Col span={12}>
                        ...
                        <Col span={12}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 12 }}>
                                ...
                                />
                                <span style={{ fontSize: 16, fontWeight: 'bold', minWidth: 30, textAlign: 'center' }}>{numChildren}</span>
                                <Button icon={<PlusOutlined />} onClick={() => setNumChildren(Math.min(20, numChildren + 1))} />
                            </div>
                        </Col>
                    </Row>
                </div>
